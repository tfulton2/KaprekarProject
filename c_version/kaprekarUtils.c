...
int isKaprekar(int n) {
  ...
  int numberOfDigits = (int) log10(square) + 1; //**changed "numDigits" to "numberOfDigits" and replaced n with square
  long modulus = 1; //**changed 0 to 1
  long first, second;
  ...
  for(i=1; i<=numberOfDigits; i++) {
    ...
    if(second > 0 && first + second == n) { //**fixed split line, moving end of if statement from 48 to 47
      return 1;
    }
  }
  return 0;
